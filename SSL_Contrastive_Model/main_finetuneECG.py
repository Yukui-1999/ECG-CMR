...
import tempfile
import os
# os.environ['TMPDIR'] = '/mnt/data/dingzhengyao/temp/'
# tempfile.tempdir = '/mnt/data/dingzhengyao/temp/'
import argparse
import datetime
import json
import numpy as np
...
from engine_finetuneECG_zheyiyaxing import train_one_epoch as train_one_epoch_zheyiyaxing, evaluate as evaluate_zheyiyaxing
...
def get_args_parser():
    ...
    parser.add_argument('--ecg_pretrained_model',default=None,type=str)
    ...
    parser.add_argument('--yangxing_classes',default=3,type=int,help='yangxing_classes')
    ...
    parser.add_argument('--max_delta', default=-0.001, type=float,
                        help='Early stopping threshold (val has to be worse than (train+delta)) (default: 0)')
    # Dataset parameters
...
def main(args):
    ...
    if args.downstream == 'regression' or  args.downstream == 'yaxing' or args.dataset == 'mutimodal_dataset_zheyi_ECG' or args.dataset == 'mutimodal_dataset_MIMIC_ECG' or args.dataset == 'mutimodal_dataset_NEWECG'or args.dataset == 'mutimodal_dataset_MIMIC_NEWECG':
        if args.downstream == 'yaxing':
            ...
            print(f'downstream:{args.downstream} latent_dim:{args.latent_dim}')
        ...
        print('no need for blanced sampler')
    else:
    ...
    if args.ECGencoder_withco:
        ...
        print('load ECGencoder without co')
    
    
    if args.ecg_pretrained_model is None:
        print('No pretrained model')
    elif 'finetuneECG' in args.ecg_pretrained_model:
    ...
    if args.freeze_ecghead:
        print('###################################')
        print('freeze ecg body')
        print('###################################')
        for name, param in model.named_parameters():
            if 'head' in name or 'fcs' in name:
                param.requires_grad = True
                print(f'param:{name} requires_grad:{param.requires_grad}')
            else:
                param.requires_grad = False
        args.max_delta = -0.0002
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if args.downstream == 'yaxing':
            train_stats = train_one_epoch_zheyiyaxing(
        ...
        if args.downstream == 'yaxing':
            test_stats = evaluate_zheyiyaxing(
