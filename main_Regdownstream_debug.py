...
from data.dataset import ECGBasePhen,CMRBasePhen,ECGPhen_wGenCMR,ECGPhen_wGenCMR_debug
...
def main(args):
    ...
    if args.input_modality == 'ECG':
        ...
        if args.use_gen_cmr:
            train_set = ECGPhen_wGenCMR_debug(txt_file=train, isTrain=True)
            valid_set = ECGPhen_wGenCMR_debug(txt_file=valid, isTrain=False)
        else:
