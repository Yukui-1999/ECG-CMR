...
from data.dataset import ECGBasePhen,CMRBasePhen,ECGPhen_wGenCMR,CMRBasePhen_gen
...
def get_args_parser():
    ...
    parser.add_argument('--gen_cmr', action='store_true', help='use generated cmr')
    # log
...
def main(args):
    ...
    if args.input_modality == 'ECG':
        ...
        train, valid = cross_validation_split(eid, k, args.fold)
        if args.gen_cmr:
            train_set = CMRBasePhen_gen(txt_file=train, isTrain=True)
            valid_set = CMRBasePhen_gen(txt_file=valid, isTrain=False)
        else:
            train_set = CMRBasePhen(txt_file=train, isTrain=True)
            valid_set = CMRBasePhen(txt_file=valid, isTrain=False)
