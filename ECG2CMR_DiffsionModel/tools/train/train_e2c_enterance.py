...
from utils.registry_class import ENGINE, MODEL, DATASETS, EMBEDDER, AUTO_ENCODER, DISTRIBUTION, VISUAL, DIFFUSION, PRETRAIN, ECGCLIPsa, ECGCMRDATASET_ECGlaCMR,ECGCMRDATASET_ECGCMR,ECGCLIP,ECGCMRDATASET_ECGlaCMRnew


@ENGINE.register_function()
def train_e2c_entrance(cfg_update,  **kwargs):
    ...
    if cfg.world_size == 1:
        print('world size == 1')
        worker(0, cfg,dataset)
    else:
        print(f'world size = {cfg.world_size}')
        mp.spawn(worker, nprocs=cfg.gpus_per_machine, args=(cfg,dataset ))
    return cfg


def worker(gpu, cfg,dataset):
    '''
    ...
    cfg.batch_size = cfg.batch_sizes
    if cfg.rank == 0:
        logging.info(f'Currnt worker with batch_size={cfg.batch_size}')
    ...
    for step in range(resume_step, cfg.num_steps + 1): 
        ...
        ecg = ecg.unsqueeze(1)
        batch_size, frames_num, _, _, _ = cmr.shape
        video_data = cmr
        ...
        with torch.no_grad():
            ...
            for chunk_data in video_data_list:
                # print(f'chunk_data: {chunk_data.shape}')#chunk_data: torch.Size([4, 3, 256, 448])  
                latent_z = autoencoder.encode_firsr_stage(chunk_data, cfg.scale_factor).detach()
                # print(f'latent_z: {latent_z.shape}')#latent_z: torch.Size([4, 4, 32, 56])
        ...
        with torch.no_grad():
            ...
            # except:
            #     pass
        # print(f'y_words.shape:{y_words.shape}')#y_words.shape:torch.Size([4, 77, 1024])
        # y_words = torch.randn(4,128,1024).cuda()
        # y_words_0 = torch.randn(4,128,1024).cuda()
        # forward
        ...
        if step == resume_step or step == cfg.num_steps or step % cfg.viz_interval == 0:
            with torch.no_grad():
                ...
                input_kwards = {
                    'model': model, 'video_data': video_data, 'step': step, 
                    'ref_frame': cmr, 'captions': ecg}
