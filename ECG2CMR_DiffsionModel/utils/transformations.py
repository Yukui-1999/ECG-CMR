...
class Normalization(object):
    ...
    Normalize the data.
    """
    def __init__(self, mode="sample_wise", groups=None) -> None:
        self.mode = mode
        self.groups = groups
    ...
    def __call__(self, sample) -> torch.Tensor:
        if self.mode == "sample_wise":
            ...
            if self.groups is None:
                raise ValueError("Group indices must be provided for group-wise normalization")

            mean = torch.Tensor()
            var = torch.Tensor()

            for group in self.groups:
                mean_group = torch.mean(sample[group], dim=-1, keepdim=True).mean(dim=0, keepdim=True)
                var_group = torch.var(sample[group], dim=-1, keepdim=True).mean(dim=0, keepdim=True)
                
                mean_group = mean_group.repeat(len(group), 1, sample.shape[-1]).squeeze()
                var_group = var_group.repeat(len(group), 1, sample.shape[-1]).squeeze()
                
                mean = torch.cat((mean, mean_group), dim=0)
                var = torch.cat((var, var_group), dim=0)

        normalized_sample = (sample - mean) / (torch.sqrt(var + 1.e-12))
