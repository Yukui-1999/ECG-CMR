...
from CMR_generation.CMR_gen_dataset import simple_generate
import yaml
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--log_dir', default='CMR_generation/example_data',
    ...
    parser.add_argument('--mar_ckpt', default="CMR_generation/model_weight/checkpoint-last.pth", type=str, help='mar model ckpt')
    parser.add_argument('--vae3d_config', default='CMR_generation/vae_kl_ft2_fs8_z16.yaml', type=str, help='vae3d model config yaml')
    parser.add_argument('--vae3d_ckpt', default="CMR_generation/model_weight/checkpoint-0004.pth", type=str, help='vae3d model ckpt')
    ...
    parser.add_argument('--use_rep_cond', default=False, type=bool, help='use_rep_cond')
    parser.add_argument('--use_mae_loss', default=False, type=bool, help='use_mae_loss')
    ...
    parser.add_argument('--ecg_model', default='stmem', type=str, help='ecg模型')
    parser.add_argument('--ecg_latent_dim', type=int, default=256, help='ecg latent dimension')
    parser.add_argument('--sample_data_path', default='CMR_generation/example_data/example_ecg.npy', type=str, 
                        help='For the sampling dataset, you can use your own ECG data, as long as it follows the format (n, 12, 5000)')
...
model = mar.__dict__[model_type](
    ...
    ecg_latent_dim=args.ecg_latent_dim,
).to(device)
# state_dict = torch.load("pretrained_models/mar/{}/checkpoint-last.pth".format(model_type))["model_ema"]
state_dict = torch.load(args.mar_ckpt,weights_only=False)["model_ema"]
# print(model)
...
def build_ecg_model(args):
    ...
    with open(os.path.realpath('CMR_generation/st_mem.yaml'), 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    config['model']['num_classes'] = args.ecg_latent_dim
    ...
    if config['mode'] != "scratch":
        ...
        if args.ecg_latent_dim is None:
            # 把checkpoint_model里面head开头的键值删掉
            for k in list(checkpoint_model.keys()):
                if k.startswith('head.'):
                    print(f"Remove key {k} from pre-trained checkpoint")
                    del checkpoint_model[k]
        # print(f"Loaded keys: {list(checkpoint_model.keys())}")
...
ecg_dataset = simple_generate(args.sample_data_path)
