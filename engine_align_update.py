...
def train_one_epoch(ECG_model: torch.nn.Module,
    ...
    ECG_model.train()
    CMR_model.eval()          # backbone 关闭 dropout/BN
    CMR_model.head.train()
    ...
    for data_iter_step, (ecg, cmr,phen) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        phen = phen.to(device, non_blocking=True)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            loss,loss_dict = criterion(ecg_outputs, cmr_outputs,phen)
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('loss_main', loss_dict['loss_main'], epoch_1000x)
            if 'loss_supcon' in loss_dict:
                log_writer.add_scalar('loss_supcon', loss_dict['loss_supcon'], epoch_1000x)
...
def evaluate(ECG_model: torch.nn.Module,
    ...
    result_list = []
    label_list = []
    for data_iter_step, (ecg, cmr,phen) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ecg = ecg.to(device, non_blocking=True)
        cmr = cmr.to(device, non_blocking=True)
        phen = phen.to(device, non_blocking=True)
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('test_loss_main', loss_dict['loss_main'], epoch_1000x)
            if 'loss_supcon' in loss_dict:
                log_writer.add_scalar('test_loss_supcon', loss_dict['loss_supcon'], epoch_1000x)
