...
def save_best_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, test_stats, evaluation_criterion):
    ...
    # 过滤掉需要保留的文件
    files_to_keep = {"log.txt", "predictions", "embeddings", 'test'}
    file_names = [f for f in file_names if f not in files_to_keep and not f.endswith('.log') and not f.startswith('events')]
    # save the 5 best performing models
    if len(file_names) > 1:
        print(file_names)
        file_names = sorted(file_names, key=lambda x: int(re.search(r'\d+', x).group()))
        files_to_remove = file_names[:-1]
